...
export async function GET(request: NextRequest) {
    try {
        ...
        // Function to fetch data, wrapped in unstable_cache
        const getDashboardData = unstable_cache(
            async (weekParam: string, scopeParam: string, email: string, filterParam: string) => {
            ...
            [`dashboard-${weekId}-${scope}-${userEmail}-${section}-${estimateFilter}`],
            { tags: [`dashboard-${weekId}`, `dashboard-${section}`], revalidate: 60 }
        );
