                ...
                <div className="space-y-6">
                    ...
                    {activeTab === 'data-scope' && selectedRole?.name !== 'Super Admin' && (
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                            {[
                                { id: 'self', icon: User, title: 'Self Only', desc: 'Users can only access records they personally own or created.' },
                                { id: 'department', icon: Building2, title: 'Department', desc: 'Access to records owned by users in the same department.' },
                                { id: 'all', icon: Globe, title: 'Global Access', desc: 'Unrestricted access to all records across the organization.' },
                                { id: 'custom', icon: Settings, title: 'Custom Rule', desc: 'Build complex logic using conditional statements.' },
                            ].map(scope => (
                                <button
                                    key={scope.id}
                                    className="p-4 border-2 border-slate-200 rounded-xl text-left hover:border-blue-300 transition-all group bg-white"
                                >
                                    <div className="w-10 h-10 rounded-lg bg-slate-100 group-hover:bg-blue-50 flex items-center justify-center mb-3 transition-colors">
                                        <scope.icon className="w-5 h-5 text-slate-600 group-hover:text-blue-600" />
                                    </div>
                                    <h4 className="font-semibold text-slate-900 mb-1">{scope.title}</h4>
