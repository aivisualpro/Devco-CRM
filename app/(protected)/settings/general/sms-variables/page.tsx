...
/* ─── Main Page ─── */
export default function SmsVariablesSettings() {
    const activeTab = 'customizations' as string;
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                                                        ...
                                                        <div className="flex flex-col gap-1.5 min-h-[40px]">
                                                            <div className="flex items-center gap-2 text-xs font-semibold text-slate-700 bg-white border border-slate-200 shadow-sm px-2.5 py-1.5 rounded-lg">
                                                                <Check className="w-3.5 h-3.5 text-emerald-500" />
                                                                Assigned Employees
                                                            </div>
                                                            {schedAlertRecipients.length > 0 && (
                                                                <div className="flex items-center gap-2 text-xs font-semibold text-amber-700 bg-amber-50 border border-amber-200 shadow-sm px-2.5 py-1.5 rounded-lg mt-1">
                                                                    <div className="w-1.5 h-1.5 bg-amber-500 rounded-full animate-pulse flex-shrink-0" />
                                                                    <span>+ {schedAlertRecipients.length} Custom Recipient{schedAlertRecipients.length !== 1 ? 's' : ''}</span>
                                                                </div>
                                                            )}
                                                        </div>
                                                    </div>
                                                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                                                        ...
                                                        <div className="flex items-center gap-2 text-xs font-semibold text-slate-700 bg-white border border-slate-200 shadow-sm px-2.5 py-1.5 rounded-lg min-h-[40px]">
                                                            <Check className="w-3.5 h-3.5 text-emerald-500" />
                                                            Auto-generated based on Title/From Date
                                                        </div>
                                                    </div>
                                                </div>

                                                {/* Recipient Dropdown (Additional to Assignees) */}
                                                <div className="space-y-4 pt-4 border-t border-slate-100">
                                                    <div className="space-y-2">
                                                        <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center justify-between">
                                                            <div className="flex items-center gap-2">
                                                                <Users className="w-3.5 h-3.5" />
                                                                Additional Recipients (along with assignees)
                                                            </div>
                                                            <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">{schedAlertRecipients.length} Selected</span>
                                                        </label>
                                                        <div className="relative" ref={schedAlertDropdownRef}>
                                                            <div
                                                                onClick={() => setIsSchedAlertDropdownOpen(!isSchedAlertDropdownOpen)}
                                                                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm flex flex-wrap gap-2 cursor-pointer hover:bg-slate-100 transition-colors bg-white shadow-sm"
                                                            >
                                                                {schedAlertRecipients.length === 0 ? (
                                                                    <span className="text-slate-400 py-1 font-semibold">Select employees from list...</span>
                                                                ) : (
                                                                    schedAlertRecipients.map(email => {
                                                                        const emp = employees.find((e: any) => e.email === email || e.value === email);
                                                                        const name = emp ? (emp.label || `${emp.firstName || ''} ${emp.lastName || ''}`.trim()) : email;
                                                                        return (
                                                                            <span key={email} className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-100 text-amber-800 text-xs font-semibold rounded-lg shadow-sm">
                                                                                {name}
                                                                                <X
                                                                                    className="w-3 h-3 hover:text-amber-500 cursor-pointer"
                                                                ...
                                                                <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl max-h-[300px] overflow-hidden">
                                                                    <div className="p-2 border-b border-slate-100">
                                                                        <div className="relative">
                                                                            ...
                                                                            <input
                                                                                type="text"
                                                                                className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-9 pr-3 py-2 text-sm outline-none focus:ring-2 focus:ring-amber-200 focus:border-amber-300 transition-all font-semibold"
                                                                                placeholder="Search employees..."
                                                                                value={schedAlertSearch}
                                                                                onChange={(e) => setSchedAlertSearch(e.target.value)}
                                                                                autoFocus
                                                                                onClick={(e) => e.stopPropagation()}
                                                                            />
                                                                        </div>
                                                                    ...
                                                                    <div className="overflow-y-auto max-h-[230px]">
                                                                        {filteredSchedAlertEmployees.length === 0 ? (
                                                                            <div className="p-4 text-xs font-bold text-slate-400 text-center">No employees found</div>
                                                                        ) : (
                                                                            filteredSchedAlertEmployees.map((emp: any) => {
                                                                                const email = emp.email || emp.value || '';
                                                                                const name = emp.label || `${emp.firstName || ''} ${emp.lastName || ''}`.trim() || email;
                                                                                const isSelected = schedAlertRecipients.includes(email);
                                                                                return (
                                                                                    <button
                                                                                        key={email}
                                                                                    ...
                                                                                    >
                                                                                        ...
                                                                                        <div className="flex-1 min-w-0">
                                                                                            <p className={`font-bold truncate ${isSelected ? 'text-amber-700' : 'text-slate-700'}`}>{name}</p>
                                                                                            <p className="text-[10px] text-slate-400 font-semibold truncate">{email}</p>
                                                                                        </div>
                                                                                        {isSelected && <Check className="w-4 h-4 text-amber-500 mr-2" strokeWidth={3} />}
                                                                                    </button>
                                                                                );
                                                                            })
                                                                        )}
