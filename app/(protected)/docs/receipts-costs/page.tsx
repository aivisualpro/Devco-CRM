...
export default function ReceiptsCostsPage() {
    ...
    const filteredEstimates = useMemo(() => {
        ...
        // Sort by Estimate Number (desc) then Version (desc)
        return res.sort((a, b) => {
    ...
    const getSelectedEstimateLabel = () => {
        ...
        return est ? `${est.estimate || 'No #'} ${est.versionNumber ? `(V${est.versionNumber})` : ''} - ${est.projectName || 'Untitled'}` : 'Select Estimate...';
    };
                                        ...
                                        >
                                            <div className="flex flex-col">
                                                <span className="font-bold flex items-center gap-2">
                                                    {est.estimate || 'No #'} 
                                                    {est.versionNumber && <span className="text-[10px] bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded-full">V{est.versionNumber}</span>}
                                                </span>
