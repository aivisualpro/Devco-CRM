...
export default function ReceiptsCostsPage() {
    ...
    const filteredReceipts = useMemo(() => {
        ...
        result.sort((a: any, b: any) => {
            ...
            // For date sorting, compare as actual Date objects
            if (sortConfig.key === 'date') {
                const dateA = valA ? new Date(String(valA).includes('T') ? valA : `${valA}T00:00:00`).getTime() : 0;
                const dateB = valB ? new Date(String(valB).includes('T') ? valB : `${valB}T00:00:00`).getTime() : 0;
