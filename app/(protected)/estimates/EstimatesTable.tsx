...
export default function EstimatesTable({ initialData }: { initialData?: any[] }) {
    const router = useRouter();
    const { toasts, success, error: toastError, removeToast } = useToast();

    const ESTIMATES_CACHE_KEY = 'devco_estimates_list_cache';
    const ESTIMATES_CACHE_TTL = 2 * 60 * 1000; // 2 minutes
    ...
    const [estimates, setEstimates] = useState<Estimate[]>(() => {
        ...
        if (typeof window !== 'undefined') {
            try {
                const cached = sessionStorage.getItem(ESTIMATES_CACHE_KEY);
                if (cached) {
                    const { data, timestamp } = JSON.parse(cached);
                    if (Date.now() - timestamp < ESTIMATES_CACHE_TTL) return data;
                }
            } catch (e) {}
        }
        return [];
        ...
        if (typeof window !== 'undefined') {
            try {
                ...
                if (cached) {
                    ...
                    if (Date.now() - timestamp < ESTIMATES_CACHE_TTL) return false;
                }
            } catch (e) {}
        }
        return true;
        ...
        try {
            ...
            if (data.success) {
                ...
                if (isLoadMore) {
                    ...
                    // Cache page 1 of default view for instant load next time
                    if (pageToFetch === 1 && !search && activeFilter === 'all') {
                        try {
                            sessionStorage.setItem(ESTIMATES_CACHE_KEY, JSON.stringify({
                                data: newEstimates,
                                timestamp: Date.now()
                            }));
                        } catch (e) {}
    ...
    // Initial Load of Supporting Data (with sessionStorage cache)
    useEffect(() => {
        ...
        fetchFresh().catch(err => console.error('Failed to fetch supporting data', err));
    }, []);
    ...
    const handleCreate = async () => {
        ...
        try {
            const currentUserEmailStr = currentUser?.email || null;

            const res = await fetch('/api/estimates', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    proposalWriter: currentUserEmailStr,
                    createdBy: currentUserEmailStr
                })
            });
            const data = await res.json();
            if (data.success && data.result?._id) {
                ...
                sessionStorage.setItem(`preload_estimate_${slug}`, JSON.stringify(data.result));
                            ...
                            <SearchInput
                                ...
                                onEnter={() => {
                                    if (filteredEstimates.length > 0) {
                                        ...
                                        sessionStorage.setItem(`preload_estimate_${slug}`, JSON.stringify(e));
                                        router.push(`/estimates/${slug}`);
                                    }
                                }}
                        ...
                        <>
                                                        ...
                                                        <div className="relative flex -space-x-1.5">
                                                            {writers.slice(0, 2).map((writer, idx) => {
                                                                ...
                                                                return emp?.profilePicture ? (
                                                                    <div key={idx} className="relative w-6 h-6 rounded-full overflow-hidden"><Image fill sizes="(max-width: 768px) 100vw, 33vw"
                                                                        src={cld(emp.profilePicture, { w: 128, q: 'auto' })}
                                                                        alt=""
                            ...
                            <div className="hidden lg:block h-full">
                                ...
                                >
                                    <TableHead>
                                        <TableRow>
                                            <TableHeader onClick={() => handleSort('estimate')} className="cursor-pointer hover:bg-gray-100 text-xs w-16">
                                                <div className="flex items-center">Est.<SortIcon column="estimate" /></div>
                                            </TableHeader>
                                            {!showFinals && (
                                                <TableHeader onClick={() => handleSort('versionNumber')} className="cursor-pointer hover:bg-gray-100 w-10 text-xs">
                                                    <div className="flex items-center">V.<SortIcon column="versionNumber" /></div>
                                                </TableHeader>
                                            )}
                                            <TableHeader onClick={() => handleSort('projectName')} className="cursor-pointer hover:bg-gray-100 text-xs w-40">
                                                <div className="flex items-center">Project<SortIcon column="projectName" /></div>
                                            </TableHeader>
                                            <TableHeader onClick={() => handleSort('date')} className="cursor-pointer hover:bg-gray-100 text-xs w-16">
                                                <div className="flex items-center">Date<SortIcon column="date" /></div>
                                            </TableHeader>
                                            <TableHeader onClick={() => handleSort('customerName')} className="cursor-pointer hover:bg-gray-100 text-xs w-40">
                                                <div className="flex items-center">Customer<SortIcon column="customerName" /></div>
                                            </TableHeader>
                                            <TableHeader onClick={() => handleSort('proposalWriter')} className="cursor-pointer hover:bg-gray-100 text-xs w-12 text-center">
                                                Writer
                                            </TableHeader>
                                            <TableHeader onClick={() => handleSort('fringe')} className="cursor-pointer hover:bg-gray-100 text-xs w-16">
                                                Fringe
                                            </TableHeader>
                                            <TableHeader onClick={() => handleSort('certifiedPayroll')} className="cursor-pointer hover:bg-gray-100 text-xs w-10">
                                                CP
                                            </TableHeader>
                                            <TableHeader className="text-xs w-20">Services</TableHeader>
                                            ...
                                            <TableHeader onClick={() => handleSort('bidMarkUp')} className="cursor-pointer hover:bg-gray-100 text-xs w-10">
                                            ...
                                            <TableHeader onClick={() => handleSort('grandTotal')} className="cursor-pointer hover:bg-gray-100 text-xs whitespace-nowrap">
                                                <div className="flex items-center">Org. Cont<SortIcon column="grandTotal" /></div>
                                            </TableHeader>
                                            <TableHeader className="text-xs whitespace-nowrap">Chg. Order</TableHeader>
                                            <TableHeader className="text-xs whitespace-nowrap">Upd. Contract</TableHeader>
                                            <TableHeader onClick={() => handleSort('status')} className="cursor-pointer hover:bg-gray-100 text-xs w-20">
                                            ...
                                            paginatedEstimates.map((est, index) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        {!showFinals && (
                                                            <TableCell>
                                                                <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium bg-gray-100 text-gray-800 border border-gray-200 shadow-sm">
                                                                    V.{est.versionNumber || 1}
                                                                </span>
                                                            </TableCell>
                                                        )}
                                                        ...
                                                        <TableCell className="text-xs max-w-[160px]">
                                                            <Tooltip>
                                                                <TooltipTrigger asChild>
                                                                    <div className="truncate text-slate-800">{getCustomerName(est)}</div>
                                                                </TooltipTrigger>
                                                                <TooltipContent>
                                                                            ...
                                                                            <div key={idx} className="relative z-0 hover:z-10 transition-all">
                                                                                {emp?.profilePicture ? (
                                                                                    <div className="relative w-8 h-8 rounded-full overflow-hidden"><Image fill sizes="(max-width: 768px) 100vw, 33vw"
                                                                                        src={cld(emp.profilePicture, { w: 128, q: 'auto' })}
                                                                                        alt={writer}
                                                                                        className="rounded-full border-2 border-white object-cover shadow-sm w-full h-full"
                                                                                        title={writer}
                                                                                    /></div>
                                                                                ) : (
                                                                                    <div
                                                        ...
                                                        <TableCell>
                                                            {est.fringe && (
                                                                <Badge {...getBadgeProps('Fringe', est.fringe)} className="text-xs px-2 py-0">
                                                                    {est.fringe}
                                                                </Badge>
                                                            )}
                                                        </TableCell>
                                                        <TableCell>
                                                            {est.certifiedPayroll && (
                                                                <Badge {...getBadgeProps('Certified Payroll', est.certifiedPayroll)} className="text-xs px-2 py-0">
                                                                    {est.certifiedPayroll}
                                                                </Badge>
                                                            )}
                                                        </TableCell>
                                                        ...
                                                        <TableCell className="font-bold text-blue-900 text-xs whitespace-nowrap">
                                                            {(() => {
                                                                const coTotal = estimates
                                                                    .filter(e => e.estimate === est.estimate && e.isChangeOrder)
                                                                    .reduce((sum, e) => sum + (e.grandTotal || 0), 0);
                                                        ...
                                                        <TableCell onClick={(e) => e.stopPropagation()}>
                                                            <div className="relative">
                                                                ...
                                                                >
                                                                    <Badge
                                                                        {...getBadgeProps('Status', est.status || 'draft')}
                                                                        className="text-xs px-2 py-0 cursor-pointer hover:opacity-80 transition-opacity flex items-center gap-1"
                                                                    >
                                                                        {['Won', 'Completed'].includes(est.status || '') && (
                                                                            <Lock className="w-2.5 h-2.5" />
                                                                        )}
