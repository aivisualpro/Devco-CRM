'use client';

import React, { useMemo } from 'react';
import { GenericLineItemsTable, LineItemColumnConfig } from '@/components/line-items/GenericLineItemsTable';

interface MiscellaneousLineItemsTableProps {
    items: any[];
    onUpdateItem?: (item: any, field: string, value: string | number) => void;
    onDelete?: (item: any) => void;
    onDuplicate?: (item: any) => void;
}

export function MiscellaneousLineItemsTable({ items, onUpdateItem, onDelete, onDuplicate }: MiscellaneousLineItemsTableProps) {
    const columns = useMemo<LineItemColumnConfig[]>(() => [
        { key: 'miscellaneous', header: 'Item', type: 'text', editable: true, width: '30%' },
        { key: 'classification', header: 'Classifications', type: 'text', editable: true, width: '25%' },
        { key: 'quantity', header: 'Qty', type: 'number', editable: true, width: '8%' },
        { key: 'days', header: 'Days', type: 'number', editable: true, width: '8%' },
        { key: 'uom', header: 'UOM', type: 'text', editable: true, width: '8%' },
        { key: 'cost', header: 'Cost', type: 'number', editable: true, width: '10%' },
        { 
            key: 'total', 
            header: 'Total', 
            type: 'currency', 
            editable: false, 
            align: 'right',
            width: '10%',
            computed: (row) => {
                const qty = parseFloat(row.quantity) || 1;
                const days = parseFloat(row.days) || 1;
                const cost = parseFloat(row.cost) || 0;
                return qty * days * cost;
            }
        }
    ], []);

    return (
        <GenericLineItemsTable
            rows={items}
            columns={columns}
            onUpdateItem={onUpdateItem}
            onDelete={onDelete}
            onDuplicate={onDuplicate}
            emptyMessage="No miscellaneous items found."
        />
    );
}
