...
export function EstimateHeaderCard({
    ...
    const [allVendorsSubs, setAllVendorsSubs] = useState<any[]>([]);
    const [loadingVS, setLoadingVS] = useState(false);
    const [vsSearchQuery, setVsSearchQuery] = useState('');
    const [vsTypeFilter, setVsTypeFilter] = useState<'All' | 'Vendor' | 'Sub Contractor'>('All');
    const [isAddingVS, setIsAddingVS] = useState(false);
    ...
    return (
        <>
            <div className="bg-[#eef2f6] rounded-2xl lg:rounded-[40px] p-2 lg:p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3 lg:gap-4">
                    ...
                    <div className="flex flex-col gap-2 p-4 rounded-2xl bg-white/30 shadow-[inset_2px_2px_6px_#d1d9e6,inset_-2px_-2px_6px_#ffffff]">
                        ...
                        <div ref={dropdownRef}>
                                        ...
                                        `}
                                            ...
                                            >
                                                {hasValue ? (
                                                    <>
                                                        <span
                                                            className={`font-black uppercase leading-none text-center px-0.5 ${(formData.fringe?.length || 0) <= 4
                                                                ? 'text-[9px]'
                                    ...
                                    {(() => {
                                        ...
                                        `}
                                            ...
                                            >
                                                {hasValue ? (
                                                    <span
                                                        className={`font-black uppercase leading-none text-center px-0.5 ${(formData.certifiedPayroll?.length || 0) <= 4
                                                            ? 'text-[9px]'
                            ...
                            {/* Row 3: Vendors/Subs, Services & Markup % — 3 column pill row */}
                            <div className="flex gap-2 mb-2">
                                {/* Vendors/Subs Column */}
                                <div className="flex-1 relative flex flex-col items-center p-2">
                                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2 block text-center leading-tight">
                                    ...
                                    {(() => {
                                        const attached: any[] = (formData.estimateVendorsSubContractors as any[]) || [];
                                        const count = attached.length;
                                        const hasValue = count > 0;
                                        return (
                                            <div
                                                id="field-vendors-subs"
                                                onClick={async () => {
                                                ...
                                                style={hasValue ? { backgroundColor: '#0F4C75' } : {}}
                                            >
                                                {hasValue ? <span className="font-bold text-lg text-white">{count}</span> : <Users className="w-5 h-5" />}
                                            </div>
                                        );
                                    })()}
                                </div>
                                    ...
                                    {(() => {
                                        ...
                                        `}
                                                ...
                                                >
                                                    {hasValue ? (
                                                        <span
                                                            className={`font-black uppercase leading-none text-center px-0.5 ${(formData.status?.length || 0) <= 4
                                                                ? 'text-[9px]'
                ...
                {/* ===== Vendors / Subs Manager Modal ===== */}
                {isVendorsSubsModalOpen && (
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
                        <div className="w-full max-w-4xl bg-white rounded-2xl border border-slate-200 overflow-hidden flex flex-col" style={{ maxHeight: '88vh' }}>
                            {/* Modal Header */}
                            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
                                <div className="flex items-center gap-3">
                                    <div className="w-9 h-9 rounded-xl bg-[#0F4C75] flex items-center justify-center">
                                        <Users className="w-4 h-4 text-white" />
                                    </div>
                                    <div>
                                        <h2 className="text-base font-black text-slate-800">Vendors &amp; Sub Contractors</h2>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Manage for this estimate</p>
                                    </div>
                                </div>
                                <button onClick={() => { setIsVendorsSubsModalOpen(false); setIsAddingVS(false); setVsSearchQuery(''); }} className="w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-700 flex items-center justify-center transition-all">
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                            ...
                            <div className="flex flex-1 min-h-0 overflow-hidden">
                                ...
                                <div className="w-[55%] border-r border-slate-100 flex flex-col">
                                    <div className="p-3 border-b border-slate-100 flex items-center gap-2 shrink-0">
                                        <input type="text" placeholder="Search..." value={vsSearchQuery} onChange={e => setVsSearchQuery(e.target.value)} className="flex-1 h-9 rounded-lg px-3 text-sm bg-slate-50 border border-slate-200 outline-none text-slate-700 font-medium placeholder:text-slate-400 focus:border-slate-400" />
                                        <div className="flex rounded-lg overflow-hidden border border-slate-200 bg-white">
                                            {(['All', 'Vendor', 'Sub Contractor'] as const).map(f => (
                                                <button key={f} onClick={() => setVsTypeFilter(f)} className={`px-2.5 py-1.5 text-[9px] font-black uppercase tracking-wider transition-all border-r border-slate-200 last:border-0 ${vsTypeFilter === f ? 'bg-[#0F4C75] text-white' : 'text-slate-500 hover:bg-slate-50'}`}>{f === 'All' ? 'All' : f === 'Vendor' ? 'Ven' : 'Sub'}</button>
                                            ))}
                                        </div>
                                        <button onClick={() => setIsAddingVS(!isAddingVS)} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all shrink-0 border ${isAddingVS ? 'bg-[#0F4C75] text-white border-[#0F4C75]' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-400 hover:text-slate-700'}`}><Plus className="w-4 h-4" /></button>
                                    </div>

                                    {isAddingVS && (
                                        <div className="p-3 bg-slate-50 border-b border-slate-200 space-y-2 shrink-0">
                                            <div className="flex gap-2">
                                                <select value={newVS.type} onChange={e => setNewVS(p => ({ ...p, type: e.target.value as any }))} className="h-9 rounded-lg px-2.5 text-xs font-bold bg-white border border-slate-200 outline-none text-slate-700 focus:border-slate-400">
                                                    <option value="Vendor">Vendor</option>
                                                    <option value="Sub Contractor">Sub Contractor</option>
                                                </select>
                                                <input type="text" placeholder="Name *" value={newVS.name} onChange={e => setNewVS(p => ({ ...p, name: e.target.value }))} className="flex-1 h-9 rounded-lg px-3 text-sm bg-white border border-slate-200 outline-none text-slate-700 font-medium focus:border-slate-400" />
                                            </div>
                                            <input type="text" placeholder="Address" value={newVS.address} onChange={e => setNewVS(p => ({ ...p, address: e.target.value }))} className="w-full h-9 rounded-lg px-3 text-sm bg-white border border-slate-200 outline-none text-slate-700 font-medium focus:border-slate-400" />
                                            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest pt-1">Contacts</p>
                                            {newVS.contacts.map((c, i) => (
                                                <div key={i} className="flex gap-1.5 items-center">
                                                    <input placeholder="Name" value={c.name} onChange={e => setNewVS(p => ({ ...p, contacts: p.contacts.map((cc, ci) => ci === i ? { ...cc, name: e.target.value } : cc) }))} className="flex-1 h-8 rounded-md px-2.5 text-xs bg-white border border-slate-200 outline-none text-slate-700" />
                                                    <input placeholder="Email" value={c.email} onChange={e => setNewVS(p => ({ ...p, contacts: p.contacts.map((cc, ci) => ci === i ? { ...cc, email: e.target.value } : cc) }))} className="flex-1 h-8 rounded-md px-2.5 text-xs bg-white border border-slate-200 outline-none text-slate-700" />
                                                    <input placeholder="Phone" value={c.phone} onChange={e => setNewVS(p => ({ ...p, contacts: p.contacts.map((cc, ci) => ci === i ? { ...cc, phone: e.target.value } : cc) }))} className="w-24 h-8 rounded-md px-2.5 text-xs bg-white border border-slate-200 outline-none text-slate-700" />
                                                    {newVS.contacts.length > 1 && <button onClick={() => setNewVS(p => ({ ...p, contacts: p.contacts.filter((_, ci) => ci !== i) }))} className="text-slate-400 hover:text-red-500 transition-colors"><X className="w-3.5 h-3.5" /></button>}
                                                </div>
                                            ))}
                                            <button onClick={() => setNewVS(p => ({ ...p, contacts: [...p.contacts, { name: '', email: '', phone: '' }] }))} className="text-[10px] font-black text-[#0F4C75] hover:text-[#0a3a5a] flex items-center gap-1 transition-colors"><Plus className="w-3 h-3" /> Add Contact Row</button>
                                            <div className="flex gap-2 justify-end pt-1">
                                                <button onClick={() => { setIsAddingVS(false); setNewVS({ type: 'Vendor', name: '', address: '', contacts: [{ name: '', email: '', phone: '' }] }); }} className="px-3 py-1.5 rounded-lg text-xs font-bold text-slate-500 hover:bg-slate-200 transition-colors">Cancel</button>
                                                <button disabled={!newVS.name || savingVS} onClick={async () => { if (!newVS.name) return; setSavingVS(true); try { const res = await fetch('/api/vendors-sub-contractors', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'create', payload: { ...newVS, contacts: newVS.contacts.filter(c => c.name) } }) }); const d = await res.json(); if (d.success) { setAllVendorsSubs(p => [d.result, ...p]); setIsAddingVS(false); setNewVS({ type: 'Vendor', name: '', address: '', contacts: [{ name: '', email: '', phone: '' }] }); toast.success('Added'); } else toast.error(d.error); } finally { setSavingVS(false); } }} className="px-4 py-1.5 rounded-lg text-xs font-black bg-[#0F4C75] hover:bg-[#0a3a5a] text-white disabled:opacity-50 transition-colors">{savingVS ? 'Saving...' : 'Save'}</button>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
                                        {loadingVS ? (
                                            <div className="flex items-center justify-center py-10 text-slate-400 text-xs font-bold gap-2"><div className="w-4 h-4 border-2 border-slate-400 border-t-transparent rounded-full animate-spin" /> Loading...</div>
                                        ) : (() => {
                                            const filtered = allVendorsSubs.filter(v => (vsTypeFilter === 'All' || v.type === vsTypeFilter) && (!vsSearchQuery || v.name?.toLowerCase().includes(vsSearchQuery.toLowerCase())));
                                            if (filtered.length === 0) return <div className="text-center py-10"><Users className="w-8 h-8 mx-auto mb-2 opacity-30 text-slate-400" /><p className="text-xs font-bold text-slate-400">No records found</p><p className="text-[10px] mt-1 text-slate-400">Click + to add</p></div>;
                                            return filtered.map((vs: any) => {
                                                const attachedList: any[] = (formData.estimateVendorsSubContractors as any[]) || [];
                                                const isAttached = attachedList.some((a: any) => a._id === vs._id);
                                                return (
                                                    <div key={vs._id} className={`p-3 rounded-xl border transition-all duration-200 ${isAttached ? 'bg-slate-50 border-slate-300' : 'bg-white border-slate-100 hover:border-slate-200'}`}>
                                                        <div className="flex items-center gap-3">
                                                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 text-white ${vs.type === 'Vendor' ? 'bg-[#0F4C75]' : 'bg-slate-600'}`}>
                                                                {vs.type === 'Vendor' ? <Building2 className="w-4 h-4" /> : <Users className="w-4 h-4" />}
                                                            </div>
                                                            <div className="flex-1 min-w-0">
                                                                <div className="flex items-center gap-1.5 flex-wrap">
                                                                    <p className="text-xs font-black text-slate-800 truncate">{vs.name}</p>
                                                                    <span className="text-[7px] font-black uppercase px-1.5 py-0.5 rounded bg-slate-100 text-slate-600 shrink-0">{vs.type === 'Vendor' ? 'VENDOR' : 'SUB'}</span>
                                                                </div>
                                                                {vs.address && <p className="text-[10px] text-slate-400 truncate flex items-center gap-1 mt-0.5"><MapPin className="w-2.5 h-2.5 shrink-0" />{vs.address}</p>}
                                                                {vs.contacts?.length > 0 && <p className="text-[9px] text-slate-500 mt-0.5">{vs.contacts[0].name}{vs.contacts[0].phone ? ` · ${vs.contacts[0].phone}` : ''}{vs.contacts.length > 1 ? ` +${vs.contacts.length - 1}` : ''}</p>}
                                                            </div>
                                                            <button onClick={() => { const cur: any[] = (formData.estimateVendorsSubContractors as any[]) || []; onHeaderUpdate('estimateVendorsSubContractors', (isAttached ? cur.filter((a: any) => a._id !== vs._id) : [...cur, vs]) as any); }} className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 font-black text-xs transition-all border ${isAttached ? 'bg-[#0F4C75] text-white border-[#0F4C75]' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-800 hover:text-white hover:border-slate-800'}`}>{isAttached ? '✓' : '+'}</button>
                                                        </div>
                                                    </div>
                                                );
                                            });
                                ...
                                {/* RIGHT: Attached to Estimate */}
                                <div className="w-[45%] bg-slate-50 flex flex-col">
                                    <div className="px-4 py-3 border-b border-slate-100 shrink-0">
                                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Attached to Estimate</p>
                                        <p className="text-2xl font-black text-[#0F4C75]">{((formData.estimateVendorsSubContractors as any[]) || []).length}</p>
                                    </div>
                                    <div className="flex-1 overflow-y-auto p-3 space-y-2">
                                        {((formData.estimateVendorsSubContractors as any[]) || []).length === 0 ? (
                                            <div className="text-center py-10"><Building2 className="w-8 h-8 mx-auto mb-2 opacity-20 text-slate-400" /><p className="text-xs font-bold text-slate-400">None attached yet</p><p className="text-[10px] mt-1 text-slate-400">Select from the list ←</p></div>
                                        ) : ((formData.estimateVendorsSubContractors as any[]) || []).map((vs: any, i: number) => (
                                            <div key={vs._id || i} className="p-3 rounded-xl bg-white border border-slate-200">
                                                <div className="flex items-start gap-2">
                                                    <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 text-white mt-0.5 ${vs.type === 'Vendor' ? 'bg-[#0F4C75]' : 'bg-slate-600'}`}>{vs.type === 'Vendor' ? <Building2 className="w-3.5 h-3.5" /> : <Users className="w-3.5 h-3.5" />}</div>
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-center justify-between gap-1">
                                                            <p className="text-xs font-black text-slate-800 truncate">{vs.name}</p>
                                                            <button onClick={() => { const cur: any[] = (formData.estimateVendorsSubContractors as any[]) || []; onHeaderUpdate('estimateVendorsSubContractors', cur.filter((_: any, ci: number) => ci !== i) as any); }} className="w-5 h-5 rounded text-slate-300 hover:text-red-400 flex items-center justify-center transition-all shrink-0 hover:bg-red-50"><X className="w-3 h-3" /></button>
                                                        </div>
                                                        <span className="inline-block text-[7px] font-black uppercase px-1.5 py-0.5 rounded bg-slate-100 text-slate-600 mt-0.5">{vs.type}</span>
                                                        {vs.address && <p className="text-[9px] text-slate-400 truncate mt-1 flex items-center gap-0.5"><MapPin className="w-2.5 h-2.5" />{vs.address}</p>}
                                                        {vs.contacts?.map((c: any, ci: number) => (
                                                            <div key={ci} className="mt-1.5 pt-1.5 border-t border-slate-100">
                                                                <p className="text-[9px] font-black text-slate-600">{c.name}</p>
                                                                {c.email && <p className="text-[9px] text-slate-400 flex items-center gap-0.5 mt-0.5"><Mail className="w-2.5 h-2.5" />{c.email}</p>}
                                                                {c.phone && <p className="text-[9px] text-slate-400 flex items-center gap-0.5"><Phone className="w-2.5 h-2.5" />{c.phone}</p>}
                                                            </div>
                                                        ))}
                                                    </div>
                                                </div>
                            ...
                            <div className="px-6 py-3 border-t border-slate-100 flex items-center justify-between shrink-0">
                                <p className="text-[10px] text-slate-400 font-bold">{((formData.estimateVendorsSubContractors as any[]) || []).length} attached · {allVendorsSubs.length} in master list</p>
                                <button onClick={() => { setIsVendorsSubsModalOpen(false); setIsAddingVS(false); setVsSearchQuery(''); }} className="px-6 py-2 rounded-lg text-sm font-black bg-[#0F4C75] hover:bg-[#0a3a5a] text-white transition-colors">Done</button>
                            </div>
                        </div>
                    </div>
                )}
