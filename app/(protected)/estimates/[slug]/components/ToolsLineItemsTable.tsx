...
function ToolsRow({
    ...
    useEffect(() => {
        setLocalValues(prev => ({
            tool: dirtyFields.has('tool') ? prev.tool : String(item.tool ?? ''),
    ...
    }, [item.tool, item.classification, item.subClassification, item.supplier, item.quantity, item.uom, item.cost, item.taxes]);

    // Calculate total: Qty × Cost (simple calculation for tools)
    ...
    const handleBlur = (field: string) => {
        if (dirtyFields.has(field)) {
            ...
            const numericFields = ['quantity', 'cost', 'taxes'];
    ...
    return (
        <tr className="hover:bg-gray-50/50 transition-colors group">
            ...
            <td className="p-1 text-xs text-gray-700" style={{ width: '15%' }}>
                <LiveInput
                    value={localValues.tool}
                    inputType="text"
                    onChange={(val) => handleChange('tool', val)}
                    onBlur={() => handleBlur('tool')}
                    placeholder="Tool"
                    inputId={`tools-${index}-0`}
                />
            </td>
...
export function ToolsLineItemsTable({
    items,
    onUpdateItem,
    onDelete
}: ToolsLineItemsTableProps) {
    if (!items || items.length === 0) {
        return (
            <div className="p-8 text-center text-gray-400 text-sm italic">
                No tools items found.
            </div>
        );
    }
