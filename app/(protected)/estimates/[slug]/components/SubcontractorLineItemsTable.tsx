...
function SubcontractorRow({
    ...
    useEffect(() => {
        setLocalValues(prev => ({
            ...
            contractor: dirtyFields.has('contractor') ? prev.contractor : String(item.contractor ?? ''),
    ...
    }, [item.classification, item.subClassification, item.contractor, item.quantity, item.uom, item.cost]);
...
export function SubcontractorLineItemsTable({
    items,
    onUpdateItem,
    onDelete,
    onDuplicate
}: SubcontractorLineItemsTableProps) {
    if (!items || items.length === 0) {
        return (
            <div className="p-8 text-center text-gray-400 text-sm italic">
                No subcontractor items found.
            </div>
        );
    }
