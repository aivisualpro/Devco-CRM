...
import { LetterPageEditor, SidebarAccordion, MyDropDown, ConfirmModal } from '@/components/ui';
import { 
    ...
    List, ListOrdered, Eraser, Plus, LayoutTemplate, FileText, RefreshCw
} from 'lucide-react';
...
interface MyProposalProps {
    ...
    proposals?: any[];  // Array of saved proposals
    selectedProposalId?: string | null; // Currently viewed proposal ID
    ...
    onDownloadPdf: () => void;
    onRefreshFromTemplate?: () => void;  // Fetch latest template
    onSelectProposal?: (proposalId: string) => void;  // Select from history
...
export function MyProposal({
    ...
    // State for update template confirmation
    ...
    return (
        <div className="bg-white rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200 flex flex-col overflow-hidden w-full h-full">
                ...
                {isEditing ? (
                    ...
                    <>
                        ...
                        <div className="flex items-center gap-3 flex-1">
                                    ...
                                    {services.length > 0 ? (
                                        <div className="flex items-center gap-1.5 flex-1 overflow-hidden">
                                            {services.slice(0, 3).map((service: string) => {
                                                ...
                                                const isInTemplate = templateServices.includes(service);
                                                return (
                                                    <span 
                                                        key={service}
                                                        className="inline-flex items-center gap-1 px-2 py-1 text-[11px] font-bold rounded-md flex-shrink-0 shadow-sm border border-transparent transition-colors"
                                                        style={{ 
                                                            backgroundColor: isInTemplate 
                                                                ? (opt?.color ? `${opt.color}15` : '#f1f5f9')
                                                                : '#fee2e2',
                                                            color: isInTemplate 
                                                                ? (opt?.color || '#475569')
                                                                : '#dc2626',
                                                            borderColor: isInTemplate 
                                                                ? (opt?.color ? `${opt.color}30` : '#e2e8f0')
                                                                : '#fca5a5'
                                                        }}
                                                        title={isInTemplate ? opt?.label || service : `${opt?.label || service} (Not in template)`}
                                                    >
                                                        {opt?.label || service}
                                                        <button
                                                            type="button"
                                            ...
                                            {services.length > 3 && (() => {
                                                ...
                                                const hasMismatch = hiddenServices.some(s => !templateServices.includes(s));
                                                return (
                                                    <span 
                                                        className="inline-flex items-center px-2 py-1 text-[10px] font-extrabold rounded-md flex-shrink-0 transition-colors"
                                                        style={{
                                                            backgroundColor: !hasMismatch ? '#eff6ff' : '#fee2e2',
                                                            color: !hasMismatch ? '#2563eb' : '#dc2626',
                                                            border: `1px solid ${!hasMismatch ? '#dbeafe' : '#fca5a5'}`
                                                        }}
                                                    >
                            ...
                            {/* Refresh from Template Icon */}
                            {onRefreshFromTemplate && (
                                <button
                                    onClick={onRefreshFromTemplate}
                                    title="Fetch Latest Template"
                            ...
                            {onSelectProposal && proposals.length > 0 && (
                                ...
                                >
                                    {proposals.map((proposal, index) => {
                                        const pId = proposal._id || proposal.generatedAt;
                                        return (
                                            <option key={pId || index} value={pId}>
                                                v{proposals.length - index}
                                            </option>
                                        );
                                    })}
                                </select>
            ...
            <div className="bg-[#e0e5ec] flex-1 flex flex-col min-h-0 overflow-hidden">
                {isEditing ? (
                    <div className="flex flex-1 items-start min-h-0 overflow-hidden">
                        ...
                        {/* Variables Sidebar */}
                        <div className="w-80 flex-shrink-0 p-4 h-full overflow-y-auto border-l border-slate-200 bg-slate-50/50">
            ...
            <ConfirmModal
                ...
                message="Are you sure you want to update the main template? This will overwrite the template's content with your current changes."
