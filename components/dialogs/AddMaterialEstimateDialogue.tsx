...
// Column configuration for Material
...
interface MaterialRowProps {
    ...
    columns: typeof MATERIAL_COLUMNS;
...
const MaterialRow = memo(({ 
    ...
    const formatValue = (col: typeof MATERIAL_COLUMNS[0], value: unknown): string => {
...
export function AddMaterialEstimateDialogue({
    ...
    const toggleSelection = useCallback((item: CatalogItem) => {
        if (editingCell || quickEditId) return; // Don't toggle when editing
        ...
        try {
            ...
            const res = await fetch('/api/webhook/devcoBackend', {
                ...
                body: JSON.stringify({
                    ...
                    payload: { type: 'material', id: item._id, item: updatedData }
                })
            });
            const result = await res.json();
                        ...
                        <table className="w-full text-[10px] text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {filteredCatalog.length > 0 ? (
                                    ...
                                    <tr>
                                        <td colSpan={MATERIAL_COLUMNS.length + 2} className="p-8 text-center text-gray-400 italic">
