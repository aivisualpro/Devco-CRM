...
export function AddToolsEstimateDialogue({
    ...
    const filteredCatalog = useMemo(() => {
        ...
        if (searchStr) {
            list = list.filter(item => {
                const name = String(item.tool || item.Tool || '').toLowerCase();
                const classification = String(item.classification || item.Classification || '').toLowerCase();
                const supplier = String(item.supplier || item.Supplier || '').toLowerCase();
    ...
    const displayCols = useMemo(() => ['tool', 'classification', 'subClassification', 'uom', 'supplier', 'cost'], []);
